...
class MysqlPdo implements DriveInterface {	
	...
	public function __construct($config = array()){
	    ...
	    try {            
            ...
            $this->link = new PDO($dns, $config['db_username'], $config['db_password']);
            $this->link->setAttribute(PDO::ATTR_PERSISTENT, $config['presistent']);  // 设置数据库连接为持久连接
			...
			$this->link->exec("SET NAMES {$config['charset']}");//设置编码  				
