...
abstract class Driver {
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                if(preg_match('/^(eq|neq|gt|egt|lt|elt)$/',$exp)) { // 比较运算
                   ...
                   //TODO E(L('_EXPRESS_ERROR_').':'.$val[0]);
                }
            }else {
                $count = count($val);
    ...
    protected function debug($start) {
        if($this->config['debug']) {// 开启数据库调试模式
            if($start) {
                //TODO G('queryStartTime');
            }else{
                $this->modelSql[$this->model]   =  $this->queryStr;
                //$this->model  =   '_think_';
                // 记录操作结束时间
                //TODO G('queryEndTime');
                //TODO trace($this->queryStr.' [ RunTime:'.G('queryStartTime','queryEndTime').'s ]','','SQL');
            }
        }
    }
